...
for csv_file in os.listdir("data/OVfour"):
    if csv_file.startswith("meas_") and "_center_scan" in csv_file:
        # Load the data from the csv file
        data = np.loadtxt(os.path.join("data/OVfour", csv_file), delimiter=",", skiprows=1)
    ...
    # How do I only analyze csv files with ND greater than 2.5
        ...
        if nd_val > 2.5:
            if "xline_scan" in csv_file:
                center, pos, current, current_err, p0 = darroch_error.profile(os.path.join("data/OVfour", csv_file), 1)
                pos_line, fitted_curve, fit_param, popt, residuals = darroch_error.fit(pos, current, current_err, p0)
                rad = popt[4]
                x_rad.append(rad)
            elif "yline_scan" in csv_file:
                center, pos, current, current_err, p0 = darroch_error.profile(os.path.join("data/OVfour", csv_file), 0)
                pos_line, fitted_curve, fit_param, popt, residuals = darroch_error.fit(pos, current, current_err, p0)
                rad = popt[4]
                y_rad.append(rad)
x_rad = np.mean(x_rad)
...
sipm_current = np.array(sipm_current) - 3.3424781548e-06  # Subtracting a small value to account for dark current
...
pde_estimate = np.mean(effective_pde_data[sorted_indices[:3]])  # Average of first 5 leftmost points
...
'''
figure, axis = plt.subplots(2, 1, figsize=(10, 12), sharex=True, constrained_layout=True)
